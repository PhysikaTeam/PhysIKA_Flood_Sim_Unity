...
Shader "TiledDirectionalFlow 1/Water" 
{
	Properties 
	{
		_MainTex ("Base (RGB)", 2D) = "grey" {}
		_FlowMap("FlowMap", 2D) = "blue" {}
	...
	SubShader 
	{
		...
		sampler2D _MainTex, _WaterNormalMap;
		samplerCUBE _SkyBox;
		...
		void surf (Input IN, inout SurfaceOutput o) 
		{
			...
			float2 flowdir = float2(_SpeedX, _SpeedY);
			//float2 flowdir = getUV(mytexFlowCoord);
			...
			//flowdir = getUV(floor(uv2 + float2(0.5, 0)));
			...
			//flowdir = getUV(floor(uv2 + float2(0, 0.5)));
			...
			//flowdir = getUV(floor(uv2 +float2(0.5,0.5)));
			rotmat = float2x2(flowdir.x, -flowdir.y, flowdir.y ,flowdir.x);
