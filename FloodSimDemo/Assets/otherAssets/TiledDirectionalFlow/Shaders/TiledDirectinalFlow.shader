...
Shader "TiledDirectionalFlow/Water" 
{
	...
	SubShader 
	{
		...
		sampler2D _MainTex, _FlowMap, _WaterNormalMap,_flowVelocity,_StateTex;
		samplerCUBE _SkyBox;
		float _FlowSpeed, _FlowTileScale, _NormalTileScale, _SpeedX, _SpeedY;
		...
		float2 classify(float2 flowdir) {
		...
		void surf (Input IN, inout SurfaceOutput o) 
		{
			...
			float2 flow = tex2D(_flowVelocity, IN.uv_StateTex).rg;
			//float speed = sqrt(flow.x * flow.x + flow.y * flow.y);
			...
			float2 uv2 = IN.uv_StateTex* texScale;
			//uv2 = floor(uv2 * 4096);
			//uv2 = floor(uv2 * 1024) / 1024;
		/*	float2 flowdir=0;
			for(int i=0;i<2;i++)
				for (int j = 0; j < 2; j++)
				{
					float2 tmp = uv2*4096 + float2(i, j);
					tmp = tmp / 4096.0;
					flowdir += classify(tex2D(_flowVelocity, tmp).rg);
				}
			flowdir = flowdir / 4;*/
			...
			flowdir = classify(flowdir);
			/*float2 flowdir = float2(0, 0);
			if (uv2.x > 0.5)
				flowdir = float2(1, 0);
			else
				flowdir = float2(0.9, 0.41);*/
		

		

			//float2 flowdir = tex2D(_flowVelocity, floor(mytexFlowCoord) / texScale).rg;
			//float2 flowdir = tex2D(_flowVelocity, IN.uv_StateTex);
			//flowdir = normalize(float2(_SpeedX, _SpeedY));
		 ...
		 //   //flowdir = flow.b * (flow.xy - 0.5);
			...
			////flowdir = tex2D(_flowVelocity, floor((mytexFlowCoord + float2(0.5, 0))) / texScale).rg;
			////flowdir = normalize(float2(_SpeedX, _SpeedY));
		 ...
		 //   rotmat = float2x2(flowdir.x, -flowdir.y, flowdir.y ,flowdir.x);
			...
			////flowdir = tex2D(_flowVelocity, floor((mytexFlowCoord + float2(0, 0.5))) / texScale).rg;
			////flowdir = normalize(float2(_SpeedX, _SpeedY));
			////flowdir = normalize(flowdir);
			////flowdir = tex2D(_flowVelocity, uv2 + float2(0.5, 0) / texScale);
			...
			////flowdir = tex2D(_flowVelocity, floor((mytexFlowCoord + float2(0.5, 0.5))) / texScale).rg;
			////flowdir = normalize(float2(_SpeedX, _SpeedY));
			////flowdir = normalize(flowdir);
			////flowdir = tex2D(_flowVelocity, uv2 + float2(0.5, 0.5) / texScale);
			//flowdir = tex2D(_flowVelocity, floor(uv2 +float2(0.5,0.5)) / 4096).rg;
			//flowdir = classify(flowdir);
			//rotmat = float2x2(flowdir.x, -flowdir.y, flowdir.y ,flowdir.x);
