...
public class RunTime_Terrain_Convertion : MonoBehaviour 
{
    ...
    Material GenerateMaterial_Basemap()
    {
        ...
        if(renderPipeline == RENDER_PIPELINE.Lightweight)
            newMaterial = new Material(Shader.Find("Lightweight Render Pipeline/Lit"));
        else
    ...
    Material GenerateMaterial_Splatmap()
    {
        ...
        else if (usedTexturesCount == 1)
        {
            ...
            Shader shader = Shader.Find(renderPipeline == RENDER_PIPELINE.Lightweight ? "Lightweight Render Pipeline/Lit" : "Legacy Shaders/Diffuse");
            if (shader != null)
            {
