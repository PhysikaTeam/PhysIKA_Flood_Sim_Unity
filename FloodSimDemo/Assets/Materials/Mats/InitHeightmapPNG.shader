Shader "Custom/InitHeightMapPNG"
{
    Properties
    {
		...
		_Bias("Bias", float) = 0
			_InitWaterHeight("WaterH",float) = 2.4
    }
    SubShader
    {
        // No culling or depth
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
				...
				float terrainH = 1.0* ( f2i(terrain.r) * 256 + f2i(terrain.g) ) / 65535;
				/*float4 water= tex2D(_WaterTex, i.uv);
				float waterH = 1.0 * ( f2i(water.r) * 256 + f2i(water.g) ) / 65535;*/
				...
				if (f2i(waterH) == 255 && terrainH<0.01)
					color.g = _InitWaterHeight;
				else
					color.g = 0.0;
